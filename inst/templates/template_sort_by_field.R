#' vl_sort_{enc}_by_field
#' 
#' Sort {enc} encoding by another field in a vega-lite spec.
#' @param spec A vega-lite spec.
{param_docs}
#' @return A modified spec
#' @seealso [vl_sort_{enc}] to sort by this encoding or a custom order, 
#' [vl_sort_{enc}_by_encoding()] to sort by another encoding
#' @export
vl_sort_{enc}_by_field <- function({arg_list}) {{
